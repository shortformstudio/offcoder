...
let lastTelemetry: ReturnType<typeof computeTelemetry> | null = null;

function marqueeify(code: string): void {
...
async function main(): Promise<void> {
  channel = new IPCChannel(
    DAEMON_WS_PORT,
    async (command: ClientCommand, reply) => {
      switch (command.type) {
        case 'org_repos': {
        ...
        case 'pull_repo': {
          ...
          try {
            ...
            if (!descriptor) {
              ...
              reply({ type: 'error', code: 'repo_missing', detail: `repo not found: ${command.repoId}` } satisfies ServerMessage);
              return;
            }
            ...
            reply({ type: 'error', code: 'repo_pull_failed', detail: String(error instanceof Error ? error.message : error) } satisfies ServerMessage);
          }
          break;
        }
        case 'fresh_project': {
          if (!command.name || !command.masterPlan) {
            reply({ type: 'error', code: 'bad_command', detail: 'name and masterPlan are required' } satisfies ServerMessage);
            return;
          }
        ...
        case 'self_fulfill': {
          ...
          if (task) {
            ...
            reply({ type: 'self_fulfilled', taskId: task.task_id } satisfies ServerMessage);
          } else {
            reply({ type: 'self_fulfilled', taskId: null } satisfies ServerMessage);
          }
          break;
        }
        default:
          reply({ type: 'error', code: 'bad_command', detail: `unknown command: ${command.type}` } satisfies ServerMessage);
      }
    },
    {
      token: WS_TOKEN,
  ...
  const prune = () => {
    try {
      const files = readdirSync(SCREENSHOT_ROOT)
        ...
        .sort((a, b) => b.t - a.t);
      for (const entry of files.slice(200)) rmSync(path.join(SCREENSHOT_ROOT, entry.f), { force: true });
    } catch (error) {
