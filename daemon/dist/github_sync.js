...
export function workspacePath(repoId) {
...
export async function registerRepo(db, descriptor) {
...
export async function syncIndex(db, repoId, localPath, defaultBranch = 'main') {
    const files = execFileSync('git', ['-C', localPath, 'ls-files'], { encoding: 'utf8' })
        .split('\n')
        .filter(Boolean);
    const tree = buildTree(files);
    const now = unixNow();
    const upsert = db.prepare(`INSERT INTO context_nodes (file_path, repo_id, ast_summary, dependencies, checksum, last_indexed)
    ...
    db.prepare(`INSERT INTO repo_registry (repo_id, local_path, default_branch, indexed_tree, last_synced)
     VALUES (@repo_id, @local_path, @default_branch, @indexed_tree, @last_synced)
     ON CONFLICT(repo_id) DO UPDATE SET
       indexed_tree = excluded.indexed_tree,
       last_synced = excluded.last_synced`).run({
        repo_id: repoId,
...
async function loadLanguage(lang) {
    ...
    try {
        ...
        let language = null;
        if (typeof container.language === 'function')
            language = container.language();
        const nested = container.typescript;
        if (!language && typeof nested?.language === 'function')
...
export async function extractSignatures(content, lang, file) {
    if (lang === 'plain') {
...
export function extractDependencies(content, lang) {
